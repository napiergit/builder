#!/usr/bin/env python3
"""
Test suite for {{cookiecutter.platform_name}} MCP Server
"""
...
from {{cookiecutter.platform_name}}_mcp_server import server, oauth, OAuthManager
...
class TestOAuth:
    ...
    def test_get_auth_url(self):
        ...
        assert "client_id={{cookiecutter.client_id}}" in url
        assert "redirect_uri={{cookiecutter.redirect_url}}" in url
...
class TestMCPServer:
    ...
    async def test_authenticate_tool(self):
        ...
        assert "{{cookiecutter.client_id}}" in result[0].text
